...
Additionally, we discuss some issues related to the Web Audio API (WAA) and frequency-based audio process, regarding latency and audio quality in pitch shifting and hoping to raise awareness over some potential (and welcome) changes in WAA.
\end{abstract}
...
Algorithms in both time and frequency domains, for Time Stretching and Pitch Shifting, are more sophisticated versions of OLA. Let $x \in \mathbb{R}^M$ be the input signal of size $M$, $\alpha \in \mathbb{R}$ be the stretching factor and $y \in \mathbb{R}^L$ be the output signal of size $L=\alpha \cdot M$. There are three main steps for OLA:
\begin{enumerate}
	\item Partition the input signal $x$ into a set of analysis frames of size N, each overlapping with the previous one in $H_a$ (i.e.: the analysis hop size) samples.
\begin{equation}
x_i(n) = x(n + i \cdot H_a), \,\, x_i \in \mathbb{R}^N
\end{equation}
	\item Apply a window $w$ to each analysis frame.
\begin{equation}
x_{w_i}(n) = w(n) \cdot x_i(n), \,\, x_i \in \mathbb{R}^N
\end{equation}
	\item Add the (synthesis) frame to the output, by overlapping it with $H_s$, the synthesis hop size, samples of the ``tail" of the output signal $y$
\begin{equation}
y = TODO
\end{equation}
\end{enumerate}
with $y_i = x_{w_i}$ for the basic OLA. The main idea of OLA is to retrieve frames from the input signal, each one overlapping with consecutive input frames in $H_a$ samples, and overlapping each windowed input frame with the output signal, in $H_s$ samples. The relation between both overlap/hop sizes determines the stretching factor:
...
\begin{sloppypar}
Phase Vocoder is a well documented \cite{phasevocoder:bernardini, dolsontutorial:phasevocoder, phavorit, laroche:phasinessbusiness, phasevocoder:pitchshift, phasevocoder:latency, dafx:2nd} algorithm used for Time Stretching \cite{dolsontutorial:phasevocoder,phavorit}, Pitch Shifting \cite{phasevocoder:pitchshift} and other audio effects like robotization \cite{dafx:2nd}. In general, it has higher computational costs than time domain methods like WSOLA but offers high quality audio, without missing transients. Each iteration of the Phase Vocoder eight steps and they are performed in-between steps 2 and 3 of OLA:
\begin{enumerate}
	\item Calculate the forward Fourier transform of $x_{w_i}$
\begin{equation}
X_{i}(n, \Omega_{k}) = \sum_{n=0}^{N} x_{w_i}(n) \cdot e^{-j\Omega_{k}kn}, X_{i} \in \mathbb{C}^N
\end{equation}
where $\Omega_{k} = \frac{2\pi k}{N}$ is the frequency center for frequency bin $k$.
...
\begin{equation}
y_{i}(n) = w(n) \cdot \sum_{k=0}^{N} Y_i(n,\Omega_k) \cdot e^{j\Omega_{k}kn}
\end{equation}
