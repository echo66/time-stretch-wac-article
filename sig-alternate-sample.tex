...
\begin{abstract}
\begin{sloppypar}
Audio time stretching and pitch shifting are operations that all major commercial and/or open source Digital Audio Workstations, DJ Mixing Software and Live Coding Suites offer. These operations allow users to change duration while maintaining the pitch and vice-versa. Such operations allow, for example, a DJ to speed up, or slow down, a song A that is being mixed into a song B, in order to align the tempo, and beats, of both songs. Unfortunately, there are few (and experimental) client-side JavaScript implementations of these two operations.
\end{sloppypar}
...
y(n+i \cdot H_s) = \left\{\begin{array}{lll}
	y_i(n),\,\,if\,i=0 \\
	y(n+i \cdot H_s) + y_i(n),\,if\,i>0
	\end{array}\right.
\end{equation}
\end{enumerate}
with $y_i = x_{w_i}$ for the basic OLA, $n \in \mathbb{I} \, \land 0 \leq n \leq N$ and $i \in \mathbb{N} \, \land \, i \cdot H_a \leq M \, \land \, i \cdot H_s \leq L$. The relation between both overlap/hop sizes determines the stretching factor:
...
\begin{figure}
	...
	\caption{Illustration of the basic OLA algorithm, showing how to apply both analysis and synthesis hops. For time compression, $H_a < H_s$. For time expansion, $H_a > H_s$.}
	\label{fig:ola}
\end{figure}
...
\begin{figure}
	\includegraphics[width=3.5in]{WSOLA}
	\caption{Illustration of the WSOLA analysis step. The synthesis step is equal to OLA. For each analysis frame \textit{i}, the analysis hop $H_{a_i}$ is calculated according to the optimal displacement $\delta_i$, obtained through cross-correlation between analysis frame \textit{i} and \textit{i-1}, for a maximum delay of $\delta_{max}$: $H_{a_i} = i \cdot H_a + \delta_i$}
	\label{fig:wsola}
\end{figure}
...
\begin{sloppypar}
Phase Vocoder is a well documented \cite{phasevocoder:bernardini, dolsontutorial:phasevocoder, phavorit, phasevocoder:latency, laroche:phasinessbusiness, phasevocoder:pitchshift, dafx:2nd} algorithm used for time stretching \cite{dolsontutorial:phasevocoder,phavorit}, pitch shifting \cite{phasevocoder:pitchshift} and other audio effects like robotization \cite{dafx:2nd}. In general, it has higher computational costs than time domain methods like WSOLA but offers high quality audio, without missing transients. Each iteration of the Phase Vocoder eight steps and they are performed in-between steps 2 and 3 of OLA:
\begin{enumerate}
...
\end{equation}
	...
	\item Due to the fact that the phase values are given in modulo $2\pi$ and, as such, phase 'jumps' will occur, we need to unwrap the phase in order to obtain a continuos phase function 
\begin{equation}
...
Soundtouch.js\footnote{https://github.com/also/soundtouch-js} is a port of the C++ library SoundTouch,\footnote{http://www.surina.net/soundtouch/} an WSOLA implementation. This library performs both time stretching and pitch shifting (through re-sampling) with some additional features: 
\begin{itemize}
...
\begin{itemize}
	...
	\item \textit{get\_ha}: returns the current analysis hop size. This is a useful function to calculate the increment to the "read head" of the input signal, when playing an audio file.
...
\begin{itemize}
	...
	\item \textit{istftCallback(Array real, Array imag, Array windowFrame, Array timeFrame)}: \textit{real} and \textit{imag} is the real and imaginary array describing a frequency frame; \textit{timeFrame} is the result of the inverse STFT. This function will be invoked when synthesizing a frequency frame, after the phase adaptation.
...
OLA-TS.js allows $O(N)$ time stretching, unlike PhaseVocoder.js, $O(N\,log_{2} N)$, and soundtouch.js, $O(N^2)$, but can introduce noticeable modulation in harmonic structures like the voice of a human singer, for frame sizes bellow 4096 samples, with a sample rate of 44100 Hz. Additionally, it may require some manual tinkering wi
th the overlap and beta parameters to reduce modulation.\footnote{We have already two default functions to adjust both parameters.} Due to the recommended frame size (4096), OLA-TS.js is adequate for applications where there are smooth adjustments in the stretching factor.

To make it simple for the reader to understand some of the main features and problems with each overviewed and/or discussed implementation, we have create table 1, listing, for each implementation, its algorithm, audio artefacts and the effect(s) implemented.
\end{sloppypar}

\section{Issues with Web Audio API, regarding time stretching and pitch shifting}
