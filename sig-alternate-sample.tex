...
\begin{equation}
x_i(n) = x(n + m \cdot H_a), \,\, x_i \in \mathbb{R}^N
\end{equation}
	\item Apply a window $w$ to each analysis frame.
\begin{equation}
x_{w_i}(n) = w(n) \cdot x_i(n)
\end{equation}
...
\begin{enumerate}
	\item Calculate the forward Fourier Transform of $x_w$
\begin{equation}
X(n, \Omega_{k}) = \sum_{n=0}^{\infty} x_{w}(n) * e^{-j\Omega_{k}kn},\,\, \Omega_{k} = \frac{2\pi}{N}	
\end{equation}
and calculate the magnitude, ($|X(n, \Omega_{k})|$) and phase ($\angle X(n, \Omega_{k})$) spectra of the frequency frame by converting $X(n, \Omega_{k})$ to polar coordinates.
...
\begin{equation}
\Delta_{\angle X_k} = \angle X(n, \Omega_{k}) - \angle X(n, \Omega_{k})
\end{equation}
\begin{equation}
\Delta_{p} = \Delta_{\angle X} - H_a * \Omega_{k}
\end{equation}
...
\begin{equation}
\Delta_{p} - 2\pi * round(\frac{\Delta_{p}}{2\pi})
\end{equation}
\begin{equation}
\omega_{k} = \Omega_{k} + \frac{}{H_a}
\end{equation}
\begin{equation}
PhaseHop = \omega_{k} * H_s
\end{equation}
